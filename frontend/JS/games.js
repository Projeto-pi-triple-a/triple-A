...
window.onload = async () => {
  const response = await fetch('http://localhost:3000/games/'+company)
    ...
    console.log("Ultimo evento: "+lastEventOfficial)
    console.log("Próximo evento: "+nextEventOfficial)
    console.log(event)

      console.log(descriptionGame)
      console.log(gameName)
      console.log(id)
      console.log(oficialDate.substring(0, 10))
      ...
      html+=`</div>`;
      html+=`<h2 class="event">Ultimo evento: ${lastEventOfficial}</h2>
      <h2 class="event">Próximo evento: ${nextEventOfficial}</h2>
      ...
      for (let i = 0; i < containersCards; i++) {//for para construir os containers dos cards
       ...
       let count=0;//contador que vai verificar quando chegar em 4 cards dentro do container e reseta por cada container
        do {
          const {descriptionGame,gameName,linkVideo,oficialDate,oficialPrice} = games[cardGamesCount]
